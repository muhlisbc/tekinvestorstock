...
function displayTekIndex(forceRefresh) {
        ...
        Discourse.ajax("/stock/get_tekindex_stocks", {
          ...
          if(data.stock.length > 0) {
            ...
            for (var i = data.stock.length - 1; i >= 0; i--) {
              ...
              if(stock.last_trade_price_only != null) {
                ...
                if($('#stock_data_tekindex').length > 0 && forceRefresh == false) { // stock data has been loaded, update existing stock numbers
                  // update data
                  console.log('updating ' + nameForUrl + ' data to: ' + last_trade_price_only + ', ' + percent_change + ' ' + nameForUrl + ', ' + last_trade_price_only);
                  //console.log($('#stock_data a[data-symbol="' + nameForUrl + '"] .stock_last'));
                  $('#stock_data_tekindex tr[data-symbol="' + nameForUrl + '"] .stock_last').numberAnimate('set', last_trade_price_only);
                  $('#stock_data_tekindex tr[data-symbol="' + nameForUrl + '"] .stock_change_percent .number-animate').numberAnimate('set', percent_change);
                  // set up or down
                  $('#stock_data_tekindex tr[data-symbol="' + nameForUrl + '"] .change_icon, #stock_data tr[data-symbol="' + nameForUrl + '"] .stock_change_percent').removeClass('positive').removeClass('negative').addClass(change_direction);
  
                }
  
                if($('#stock_data_tekindex').length == 0 || forceRefresh) { // stock data has not already been loaded
                  template = template + '<tr data-symbol="' + nameForUrl + '"><td class="td-ticker"><a href="/tags/' + nameForUrl.toLowerCase() + '"><span class="stock_symbol">' + nameForUrl + '</span></a></td><td class="td-last"><span class="stock_last number-animate">' + last_trade_price_only + '</span></td><td class="td-change"><span class="stock_change_percent ' + change_direction + '"><span class="number-animate">' + percent_change + '</span>%</span></td></tr>';
                }
          ...
          if($('#stock_data_tekindex').length == 0 || forceRefresh) { // stock data has not already been loaded
            ...
            if(forceRefresh) {
              $('#stock_data_tekindex').remove();
              //console.log('removing stock data');
            }

            stock_html = '<div id="stock_data_tekindex"><div class="container"><table id="stock_data_inner"><thead><th class="th-symbol">Ticker</th><th class="th-last">Siste</th><th class="th-change">I dag</th></thead><tbody>' + template + '</tbody></table></div></div>';
            //console.log(stock_html);

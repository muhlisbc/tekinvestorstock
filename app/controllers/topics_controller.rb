...
class TopicsController < ApplicationController
  ...
  def show
    ...
    #puts params[:topic_id]
      ...
      #end
      #puts "cat:"
      #puts topic.category.slug
      #puts topic
      if topic.category.slug == "insider"
        ...
        if !current_user.nil?
          ...
          if group && GroupUser.where(user_id: current_user.id, group_id: group.id).exists? 
            #is insider
            puts "is insider, show topic"
            perform_show_response    
            #url = "https://www.tekinvestor.no/insider?u=" + current_user.name
            #redirect_to url
         else
            # not insider, redirect
            puts "not insider, redirect to sales page"
            url = "https://www.tekinvestor.no/insider?u=" + current_user.name
            redirect_to url
          end
